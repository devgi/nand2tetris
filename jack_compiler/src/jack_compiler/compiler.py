...
from jack_compiler.consts import (CLASS, FIELD, STATIC, BOOLEAN, CHAR,
                                         INT, CONSTRUCTOR, FUNCTION, METHOD, VOID, VAR, LET, IF, WHILE, DO,
                                         RETURN, ELSE, TRUE, FALSE, NULL, THIS, OPS, UNARY_OPS)
from jack_compiler.tokenizer import (tokenize, Keyword, Symbol,
                                            Identifier, Integer, String)

from jack_compiler.symbol_table import SymbolTable, SubroutineSymbolTable
from jack_compiler.vm_writer import VMBytecodeWriter
...
class JackCompiler(object):
    ...
    def compile(self, jack_file_content):
        tokens = tokenize(jack_file_content)
        return JackFileCompiler(tokens, self._symbol_table).process()


class JackFileCompiler(object):
    """
    ...
    def _process_expression(self):
        with self._write_parsing_rule("expression"):
            ...
            while self._is_next_token(Symbol, *OPS):
    ...
    def _process_term(self):
        with self._write_parsing_rule("term"):
            ...
            # Handle array indexing:
                ...
                self._vm_bytecode.write_array_indexing(symbol)
